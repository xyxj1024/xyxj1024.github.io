...
In this post, I would like to describe a Standard ML implementation of **binary search tree (BST)**{: style="color: red"}. Each tree **node**, if not empty, records information about its own **value** along with its **descendant(s)**. A client should be able to
<ol type="a">
	<li><code>find</code> a node value using a **key**;</li>
	<li><code>insert</code> values onto an existed tree;</li>
	<li><code>remove</code> nodes from an existing tree;</li>
	<li>perform tree traversal using higher order functions.</li>
</ol>
The BST is kept as general as possible such that the values and keys can be any types.
...
The <code>insert()</code> function returns a pair containing the new tree and an optional type of the replaced element. If the key provided matches an existed node in the tree, the old element is replaced.
