...
<span>Total labor rents = union rents + industry rents + firm size rents</span>
...
$$\log \textrm{wage}_{t} - \log \textrm{wage}_{t-1} = \alpha + \beta\,\textrm{slack}_{t} + \gamma\,\textrm{lagged inflation}_{t} + \epsilon_{t},$$
...
$$
\begin{align}
\textrm{Predicted unemployment rate}_{t} = \alpha \\
+ \beta \sum\limits_{0}^{12} \log \textrm{vacancy rate}_{t - i} \\
+ \delta \sum\limits_{0}^{12} \log \textrm{quits rate}_{t - i} \\
+ \gamma\,\textrm{time trend}_{t} \\
+ \theta\,\textrm{structural break}_{t} \\
+ \epsilon_{t},
\end{align}
$$
...
$$\pi_{t} = \underbrace{\sum\limits_{i}(1 - \mathbb{I}_{i \in f, e}) \omega_{i} \pi_{i, t}}_\textrm{core} + \underbrace{\sum\limits_{i}\mathbb{I}_{i \in f, e} \omega_{i} \pi_{i, t}}_\textrm{non-core},$$
...
\mathbb{I}_{i \in f, e} =
\begin{cases}
     1, & \textrm{if equals energy (e) or food (f)},\\
     0, & \textrm{otherwise}.
\end{cases}
$$
