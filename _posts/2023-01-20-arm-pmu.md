...
The `on_each_cpu()` function call inside the `ccnt_get_init()` method, needed for SMP architecture, disables preemption according to its implementation (find more details in the [RCU Handbook](https://docs.kernel.org/RCU/rcubarrier.html)). To build the module, issue the command:
```bash
