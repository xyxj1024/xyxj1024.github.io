...
<pre>Total labor rents = union rents + industry rents + firm size rents</pre>
...
$$\log \texttt{wage}_{t} - \log \texttt{wage}_{t-1} = \alpha + \beta\,\texttt{slack}_{t} + \gamma\,\texttt{lagged inflation}_{t} + \epsilon_{t},$$
...
$$
\begin{align}
\texttt{Predicted unemployment rate}_{t} = \alpha \\
+ \beta \sum\limits_{0}^{12} \log \texttt{vacancy rate}_{t - i} \\
+ \delta \sum\limits_{0}^{12} \log \texttt{quits rate}_{t - i} \\
+ \gamma\,\texttt{time trend}_{t} \\
+ \theta\,\texttt{structural break}_{t} \\
+ \epsilon_{t},
\end{align}
$$
...
$$\pi_{t} = \underbrace{\sum\limits_{i}(1 - \mathbb{I}_{i \in f, e}) \omega_{i} \pi_{i, t}}_\texttt{core} + \underbrace{\sum\limits_{i}\mathbb{I}_{i \in f, e} \omega_{i} \pi_{i, t}}_\texttt{non-core},$$
...
\mathbb{I}_{i \in f, e} =
\begin{cases}
     1, & \texttt{if equals energy (e) or food (f)},\\
     0, & \texttt{otherwise}.
\end{cases}
$$
