...
**Definition 2.** Let $$G$$ be a group and let $$g \in G$$. The **order** of $$G$$ is its cardinality $$|G|$$. The order of $$g$$ in $$G$$ is the smallest positive integer $$n$$ such that $$g^{n} = e$$ provided that such an integer exists. If no such positive integer $$n$$ exists, then the order of $$g$$ is infinite.
...
1. $$\langle g \rangle = \{ g^{k} | k \in \mathbb{Z} \}$$;
2. If $$|g| = \infty$$, then the elements $$g^{k}, k \in \mathbb{Z}$$ are all distinct, $$|\langle g \rangle| = \infty$$;
3. If $$|g| = n$$, then for $$k, l \in \mathbb{Z}$$, $$g^{k} = g^{l}$$ is equivalent to $$k = l \pmod{n}$$, and

$$\langle g \rangle = \{ g^{k} | k \in \mathbb{Z} \} = \{ e, g, g^{2}, \dots , g^{n - 1} \}$$

with $$|\langle g \rangle| = n$$. In particular, for $$k \in \mathbb{Z}$$, $$g^{k} = e$$ is equivalent to $$n | k$$.
