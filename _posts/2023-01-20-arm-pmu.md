...
Calling this function inside the `enable_ccnt()` function, my userspace program looks like this:
...
The Linux kernel provides indirect access to hardware counters, including the ARM PMU, via the [`perf_event_open()` system call](https://man7.org/linux/man-pages/man2/perf_event_open.2.html) with system call number defined in `/arch/arm64/include/asm/unistd32.h`:
...
When the `pid` argument is zero and the `cpu` argument is minus one, the system call measures the calling process/thread on any CPU. A single event on its own is created with `group_fd = -1` and is considered to be a group with only one member.
