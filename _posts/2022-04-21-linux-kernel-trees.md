...
```
The function <code>__rb_insert()</code> checks and maintains the invariant properties of red-black tree. By default, the node to be inserted is colored red. Mere recoloring is performed when both parent and uncle of the inserted node are red:
```text
    gparent(black)
     /          \
