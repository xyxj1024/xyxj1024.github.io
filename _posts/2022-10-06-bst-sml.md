...
[^1]: Alternatively, we may want to implement a **red-black tree**{: style="color: red"}, in which case a non-empty node takes on the information about its own color:
```sml
datatype color = Red | Black
datatype 'e node = Nil | Sub of color * 'e * ('e node) * ('e node)
```
Specifically, a red-black tree is a height-balanced binary search tree with the color invariant:
