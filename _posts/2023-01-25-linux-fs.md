...
[^1]: Before one can access the files on a filesystem, he or she needs to mount the filesystem. When one mounts a filesystem, he or she attaches that filesystem to a directory (mount point) and makes it available to the system. The root (`/`) filesystem is always mounted.