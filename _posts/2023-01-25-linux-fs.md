...
This post is some writeup for
- CSE 422S: "Operating Systems Organization", Studio 13: "VFS Layer", and
- CSE 522S: "Advanced Operating Systems", Studio 4: "Observing File System Events", Exercises 2 through 4.

Please go to personal websites of [Dr. Christopher D. Gill](https://www.cse.wustl.edu/~cdgill/) and [Dr. James Orr](https://www.cse.wustl.edu/~james.orr/) for more information about these courses.
...
I gave a quick recap on hard links in Linux [here](https://xingjianxuanyuan.github.io/posts/seedlabs/race-condition#linux-filesystem-hard-links-vs-soft-links).
...
According to [this RHEL document](https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/8/html/system_design_guide/assembly_mounting-file-systems_system-design-guide):
...
Let's try out some bind mount commands:
...
[^1]: Before we can access the files on a filesystem, we need to mount the filesystem. When we mount a filesystem, we attache that filesystem to a directory (mount point) and make it available to the system. The root (`/`) filesystem is always mounted.