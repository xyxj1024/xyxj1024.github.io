...
This post is some notes taken from Andrew W. Appel's 1992 book, *Compiling with Continuations*[^1]. A **continuation**{: style="color: red"} is a function that expresses "what to do next"[^2]. **Continuation-passing style (CPS)**{: style="color: red"} is a program notation with clean semantics that makes every aspect of control flow and data flow explicit. It is considered as a good abstract representation to use for compilation and optimization.
