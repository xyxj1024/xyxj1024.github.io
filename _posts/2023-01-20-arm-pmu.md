...
For calculating a $$64$$-bit power of any given number, we can use the `int_pow()` function; for an integer approximation of the square root, we can use the `int_sqrt()` or `int_sqrt64()` function. Both of them are declared in `<linux/kernel.h>`:
...
Henceforth, my userspace program looks like this:
...
The Linux kernel provides indirect access to hardware counters, including the ARM PMU, via the `perf_event_open()` system call declared in `/arch/arm64/include/asm/unistd32.h`:
...
[This blog post](https://community.arm.com/arm-community-blogs/b/embedded-blog/posts/using-the-arm-performance-monitor-unit-pmu-linux-driver) provides some helpful instructions. In this exercise, we are asked to create a program directly on the Raspberry Pi that uses this system call to get a file descriptor which, when read, supplies the current value of the `PMCNTR` register.
