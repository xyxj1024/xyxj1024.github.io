...
In this exercise, we are asked to create a program directly on the Raspberry Pi that uses this syscall to get a file descriptor which, when read, supplies the current value of the `PMCNTR` register. [This ARM blog post](https://community.arm.com/arm-community-blogs/b/embedded-blog/posts/using-the-arm-performance-monitor-unit-pmu-linux-driver) provides some helpful instructions. Use `PERF_COUNT_HW_CPU_CYCLES` as the `config` field of the `perf_event_attr` argument to the syscall. Then, similarly to the previous exercises, take $$100$$ samples of the elapsed cycles between two subsequent `read`s, then have our program print the minimum, maximum, mean, and standard deviation of these samples.
