...
Continuation functions are bound by ordinary `FIX` operators and passed as arguments to functions. The effect of evaluating `FIX`$$(\vec{f}, E)$$ is to define the functions in the list $$\vec{f}$$ and then to evaluate $$E$$, which (usually) calls one or more of the $$f_{i}$$ by means of the `APP` operator, or passes one of the $$f_{i}$$ as an argument to another function, or stores some of the $$f_{i}$$ in a data structure.